...
export class PayComponent implements OnInit {
  ...
  signDets: {
    ...
    font_size: number;
    combine: boolean;
    center_character_origin: boolean;
    phone: string;
    email: string;
  } = {
    font: null,
  ...
  download(filename, text) {
    ...
    element.setAttribute(
      "href",
      "data:application/dxf;charset=utf-8," + encodeURIComponent(text)
    );
  ...
  makefile() {
    opentype.load("./assets/DejaVuSans.ttf", (err, font) => {
      ...
      const sign: any = this.makerService.make(
        this.font,
        this.convertString(this.text),
        72,
        false,
        false
      ); // this.font, 'שלום', 72, false, false);
